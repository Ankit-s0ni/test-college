...
const ApprovalsSection = ({ data }: { data: Approvals }) => {
  ...
  const approvalCards = data.items.map((item, i) => {
    const hasTooltip = item.fullName || item.description || item.validFrom || item.validUntil;
    const isClickable = item.website;

    const content = (
      <div className="flex flex-col items-center justify-between h-32 w-40 bg-[#FFF6E8] overflow-hidden p-2 hover:shadow-md transition-shadow cursor-pointer">
